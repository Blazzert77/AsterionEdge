...
(() => {
  ...
  function availablePages(){return ['Home','Combat','Systems','FPS','Settings','Bindings'];}
  ...
  function render(){if(['Settings','Bindings'].includes(page)&&renderKey&&$('content').childElementCount&&document.activeElement?.matches('input,select')){updateHeader();return;}const key=JSON.stringify([page,connected,state?.running,state?.context,state?.actions,state?.feed,state?.location,state?.shard,state?.build,state?.ship,state?.appearance,state?.update,state?.economy,powerTab,targetTab]);if(key===renderKey)return;if(hold){updateHeader();return;}renderKey=key;cancelHold();nav();updateHeader();const c=$('content');c.replaceChildren();if(page==='Home')home(c);else if(page==='Combat')groupPage(c,'COMBAT & CIBLAGE','Les commandes utiles au combat, regroupées sans doublons.',combatGroups);else if(page==='Systems')groupPage(c,'SYSTÈMES DU VAISSEAU','Pilotage avancé, énergie, boucliers et systèmes secondaires.',systemGroups);else if(page==='FPS')footPage(c);else if(page==='Bindings')bindingsPage(c);else settings(c);}
